...
install_build_tools_linux() {
    ...
    if command -v pacman &> /dev/null || is_arch_linux; then
        if is_root; then
            run_quiet_step "Installing build tools" pacman -S --noconfirm base-devel python make cmake gcc
        else
            run_quiet_step "Installing build tools" sudo pacman -S --noconfirm base-devel python make cmake gcc
        fi
        return 0
    fi
...
ensure_macos_node22_active() {
    ...
    ui_error "Node.js was installed but this shell is using ${active_version} (${active_path})"
...
install_node() {
    if [[ "$OS" == "macos" ]]; then
        ...
        if command -v pacman &> /dev/null || is_arch_linux; then
            ...
            if is_root; then
                run_quiet_step "Installing Node.js" pacman -S --noconfirm nodejs npm
            else
                run_quiet_step "Installing Node.js" sudo pacman -S --noconfirm nodejs npm
            fi
            ui_success "Node.js installed"
            print_active_node_paths || true
            return 0
...
install_git() {
    if [[ "$OS" == "macos" ]]; then
        ...
        if command -v apt-get &> /dev/null; then
            ...
            if is_root; then
                run_quiet_step "Installing Git" pacman -S --noconfirm git
            else
                run_quiet_step "Installing Git" sudo pacman -S --noconfirm git
            fi
        elif command -v dnf &> /dev/null; then
            if is_root; then
